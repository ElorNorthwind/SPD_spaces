...
function ReplaceSpaces() {
    ...
    contents = contents.replace(/((?:^| )[Гг](?:ор)?(?:од)?а?\.?|(?<=[\"«])[Оо]б?|№|(?:^| )[Чч](?:аст)?ь?[ию]?\.?|(?:^| )[Сс]т(?:ат)?ь?и?(?:ей)?я?.?|(?:^| )[Пп](?:ункт)?[уа]?(?:ом)?\.?)( )/g, "$1^"); //Правила ДО
    ...
    contents = contents.replace(/( )([гА-Я]\.[рА-Я]\.|\d{2}\:\d{2}|чел)/g, "^$2"); //Правила ПОСЛЕ
